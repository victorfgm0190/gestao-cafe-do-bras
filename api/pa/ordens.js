  ...
  try {
    ...
    for (const it of calc.itens) {
      ...
      const est = await sql`
        ...
        VALUES (${pa?.id ?? null}, ${formatarGramatura(it.gramatura)}, ${it.quantidade}, ${it.custoUnitarioTotal},
      ...
      const mov = await sql`
        ...
        VALUES (${ordemId}, ${data}, ${TIPOS_MOV.ENTRADA}, ${pa?.id ?? null}, ${pa?.nome || ''},
                ${formatarGramatura(it.gramatura)}, ${it.quantidade}, ${it.custoUnitarioTotal}, ${it.custoTotalGramatura})
        RETURNING id
      `
      itensOrdem.push({
